from eagle_eye.computer_vision.sort import Sort
from eagle_eye.computer_vision.homography import Homography
from eagle_eye.data_collector.data_collector import DataCollector
...
class BirdViewVideoCreator:
    ...
    def create_video(self):
        ...
        while True:
            ...
            self.data_collector.players_data_arr.append({f"frame{frame_count}" : player_datas})
            frame_count += 1

        self.data_collector.create_dataset(self.args["output"])
        cv2.waitKey(0)
        vs.release()